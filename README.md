...
- **Startups**: YC-first strategy for exact `employee_count` extraction, canonicalizing `Entity Name` (stripping legal suffixes like LLC, Corp, Ltd) while preserving verbatim `Raw Entity Name` and source-derived text ranges (`11-50`).
...
- **Anti-Hallucination Conservative Entity Resolution**: Strictly leaves `startup_name` and `canonical_id` blank (`""`) with `confidence = 0.0` and `Resolution Tier = UNRESOLVED` when resolution confidence is < 0.8, refusing to fabricate false vendor mappings.
- **Expanded Corporate Alias Graph**: Pre-seeded with corporate developer aliases (`Microsoft Corporation` -> `Microsoft`, `Google LLC` -> `Google`, `Meta Platforms, Inc.` -> `Meta`, `Alibaba Cloud Computing Ltd.` -> `Alibaba Cloud`, `Stability AI Ltd.` -> `Stability AI`).
