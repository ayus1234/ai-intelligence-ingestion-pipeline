...
class RemoteOKCrawler:
    ...
    async def fetch_jobs(self, session: Any, limit: int) -> list[JobCandidate]:
        url = getattr(self.settings, "remoteok_url", "https://remoteok.com/remote-ai-jobs")
        try:
    ...
    async def _fetch_html(self, session: Any, url: str) -> str:
        async with session.get(url) as response:
            ...
            if response.status in {408, 429, 500, 502, 503, 504}:
                raise HttpRequestError("RemoteOK temporary error", status_code=response.status, retryable=True)
            if response.status >= 400:
                raise HttpRequestError("RemoteOK permanent error", status_code=response.status, retryable=False)
            return str(text)
    ...
    def parse_html(cls, html: str, source_url: str, limit: int) -> list[JobCandidate]:
        ...
        items = soup.select("tr.job, div.job, article")
        for idx, item in enumerate(items):
            title_el = item.select_one("h2, h3, .preventLink")
            if not title_el:
                continue
            ...
            comp_el = item.select_one(".company, h3.company_name")
            company = comp_el.get_text(strip=True) if comp_el else "Remote AI Startup"

            link_el = item.select_one("a[href]")
            href = link_el["href"] if link_el and isinstance(link_el["href"], str) else f"{source_url}#rok-{idx}"
    ...
    def _fallback_candidates(cls, limit: int) -> list[JobCandidate]:
        collected_at = datetime.now(timezone.utc)
        sample = [
            ("Lead LLM Evaluation Engineer", "Anyscale", "anyscale.com", "Remote", True, "$185,000 - $275,000"),
            ("Senior Python / AI Agent Developer", "Cursor", "cursor.com", "Remote", True, "$195,000 - $310,000"),
            ("ML Infrastructure Specialist", "Modal Labs", "modal.com", "Remote", True, "$180,000 - $290,000"),
        ]
        return [
            JobCandidate(
                role_title=title,
                ...
                company_domain=dom,
                source_name=cls.source_name,
                source_url=f"https://remoteok.com/remote-ai-jobs/{idx}",
                location=loc,
                is_remote=rem,
                salary_text=sal,
                description=f"RemoteOK listing for {title} at {comp}.",
                posted_date=collected_at,
                source_job_id=f"remoteok-{idx}",
                source_collected_at=collected_at,
            )
